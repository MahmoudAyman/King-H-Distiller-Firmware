...
// Pin 9 for Left, Pin 10 for Right
#define AUDIO_L_PIN 9
#define AUDIO_R_PIN 10

MockAudioManager audio(AUDIO_L_PIN, AUDIO_R_PIN);

void setup() {
    Serial.begin(9600);
    audio.begin();
    
    Serial.println("Testing Stereo Speakers & PAM8403...");
    audio.play();
    
    // Using low volume because of the 2-ohm speakers
...
void loop() {
    ...
    if (millis() - lastAction > 5000) {
        audio.next(); // This will beep L then R
        lastAction = millis();
    }
}